 ...
 *
 * 수정 이력(2026-09-11): "한영자동변환키를 누르면 입력값과 무관하게 엉뚱한
 * 문자열이 재입력된다"는 제보에 따라 자기-캡처 피드백 방지 로직
 * (is_replaying 플래그 + k_work_delayable 가드)을 한 번 추가했었으나,
 * 그 수정 이후 "스위치가 완전히 무반응"이라는 새 회귀가 발생함 - 그리고
 * KSN-1(ksn1_word_flip.c, 이 파일과 변경 전 100% 동일한 코드)은 실제
 * 기기에서 이 기능이 정상 동작한다고 확인됨. 따라서 그 수정이 원인을
 * 잘못 짚은 것으로 판단, KSN-1과 동일한(실제 검증된) 이 버전으로 되돌림.
 * "VCSDLL" 증상의 진짜 원인은 재조사가 필요함 - 펌웨어 자체 로직 문제가
 * 아니라 당시 테스트했던 바이너리가 최신 소스를 반영하지 못했을 가능성
 * (이 코드는 한 번도 west 빌드로 직접 검증된 적이 없었음, 아래 2026-09-06
 * 주석 참고) 등을 우선 점검할 것.
 *
